...
class DataCollatorForTokenClassification(DataCollatorMixin):
    ...
    @staticmethod
    def attention_masking_2d(mask, sequence_length):
        for i in range(len(mask)):
            mask[i] = mask[i] + [0] * (sequence_length - len(mask[i]))
        mask.extend([[0] * sequence_length] * (sequence_length - len(mask)))
        return mask
    ...
    def torch_call(self, features):
        ...
        input_ids, attention_mask, position_ids, eos_ids, eos_attention_mask, labels = [], [], [], [], [], []
        token_type_ids = []
        for f in features:
            input_ids.append(torch.tensor(f["input_ids"]))
            position_ids.append(torch.tensor(f["position_ids"]))
            eos_ids.append(torch.tensor(f["eos_ids"]))
            eos_attention_mask.append(torch.tensor(f["eos_attention_mask"]))
            labels.append(torch.tensor(f["labels"]))
            attention_mask.append(torch.tensor(f["attention_mask"]))
            token_type_ids.append(torch.tensor(f["token_type_ids"]))

        input_ids = pad_sequence(input_ids, padding_value=self.tokenizer.pad_token_id, batch_first=True)
        bsz, sequence_length = input_ids.shape
        # padding 2D attention mask
        # for i in range(bsz):
        #     attention_mask.append(self.attention_masking_2d(features[i]["attention_mask"], sequence_length))
        attention_mask = pad_sequence(attention_mask, padding_value=0, batch_first=True)
        token_type_ids = pad_sequence(token_type_ids, padding_value=0, batch_first=True)
        position_ids = pad_sequence(position_ids, padding_value=0, batch_first=True)
        eos_ids = pad_sequence(eos_ids, padding_value=0, batch_first=True)
        eos_attention_mask = pad_sequence(eos_attention_mask, padding_value=0, batch_first=True)
        labels = pad_sequence(labels, padding_value=self.label_pad_token_id, batch_first=True)
        ...
        batch = {
            ...
            "eos_ids": torch.tensor(eos_ids, dtype=torch.int64),
            "eos_attention_mask": torch.tensor(eos_attention_mask, dtype=torch.int64),
            "labels": torch.tensor(labels, dtype=torch.int64)
        }
        return batch

    def tf_call(self, features):
        import DataCollatorForTokenClassificationtensorflow as tf
